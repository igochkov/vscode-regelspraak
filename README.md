...
Write a reglement and the rules that compute it in **one document**, and let the worked example under each bepaling prove it — with the editor support you expect from a programming language: colour that reflects meaning, errors while you type, model-aware completion, navigation and rename that follow what a name means across every file, and a run you can look inside.
...
that computes it under one another, the rule naming the lid it renders, the
worked example that checks the bepaling — and turns red the moment the half cent
rounds the wrong way — and four lines of comment that draw the whole staffel.
Over [samples/sample-wet/](samples/sample-wet).
...
of mistake the model catches, a testset going red, **Leg uit** opening the whole
derivation behind it, the coverage of the regelversies down to the gutter,
navigation and rename across file boundaries, and the formatter. Over
[samples/workspace/single-folder/](samples/workspace/single-folder).

## Features

### The reglement, and what proves it

- **A reglement as a notebook.** In juridische modus an `.rgs.md` document opens as cells: the text reads as text, the rules stand between the paragraphs and get everything below — colour, checks, completion, navigation, rename, formatting — and the worked example under a bepaling has a run button that reports each `Verwacht` line under the cell. Press the one on a rule cell and it says so: what you run is the worked example, not the rule. **Reglement bekijken** renders the whole artikel as one document, and *Openen met → Teksteditor* shows the Markdown underneath, because that is all it ever was. One notebook is one artikel — the unit a wijzigingsbesluit changes — and the folders above it are the wet's own hoofdstukken, so the explorer is its table of contents and the audit question (*what have we not implemented?*) is answered by reading down the numbering.
- **A `// Visualisatie:` comment draws what a rule does, beside the rule.** A reader of artikel 8 can see the staffel written out and cannot see that its risers grow towards the top, or that one day of geboortedatum between 20 and 21 is € 3,00 an hour. A testgeval that seats sixteen personen on eight geboortedata *is* the staffel and the run already computes every one of those values, so four comment lines above it say what to draw with them and the cell answers with a figure above its verdict:
