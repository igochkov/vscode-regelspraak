...
- **Run from the text, and see what the run computed.** A link above every testset and every testgeval runs it — the same run the Testing view makes, with the same result on the same item. **Uitkomst van dit testgeval tonen** opens a read-only panel beside the testset: the expectations with what they actually got, the values you gave and the values the model derived, the characteristics, the faults, and the derivation trace — one line per write, in order, naming the rule that made it and the operands it read. Passing and failing are coloured apart in your own theme's colours; a trace line clicks through to the rule that wrote it and a failing expectation to the `Verwacht` line that made it; a write's operands fold away until you want them. All of it in RegelSpraak's own notation, because it is computed on the server, and **Als tekst openen** gives you the same thing as text to paste.
