...
| ✅ | **Test language** (current) | `*.test.rgs` testsets as part of the language: colour, checks (`RS951`–`RS958`), outline, folding, formatting, completion, hover, and navigation and rename that cross between a testset and the model it tests |
| ⬅ | **Execution** (next) | Run rules, decision tables and testsets against their data · Results and derivation traces in-editor · Testsets as tests in the Test Explorer |
