...
| ✅ | **Importing from ALEF, and the arithmetic behind a value** (current) | The provision a rule renders, as a `// Bron:` citation a hover follows · Plurals a model need not spell out · **Importeren uit ALEF**: an ALEF project read and written out as RegelSpraak text, with a report of everything it could not translate · An apostrophe in a name, and three more words a name may contain · The arithmetic inside a rule — in the trace, in Watch, and under <kbd>F11</kbd> |
