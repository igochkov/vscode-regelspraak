...
| `regelspraak.execution.cacheExternalData` | `true` | Keeps the deliveries a `Gegevensbron` binding reads in `.regelspraak/cache` under the model root, so a later run does not parse them again. A cache file is named after the delivery's content, and files for a delivery the manifest no longer names are removed. Off writes nothing into the workspace; a run answers the same, only slower. |
